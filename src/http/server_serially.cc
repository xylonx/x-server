...
void HTTPServer::Serve(const char *addr, int port, const char *root, const char *index) {
    ...
    while (true) {
        int connfd = accept(listen_fd, reinterpret_cast<struct sockaddr *>(cli_addr_), cli_socklen_);
        if (connfd < 0) {
            spdlog::error("failed accept conn {}: {}", connfd, std::strerror(errno));
            continue;
        }
        spdlog::info("accept new connection {}", connfd);
